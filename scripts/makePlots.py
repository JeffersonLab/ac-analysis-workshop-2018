...
ax0p1 = ax0.twinx()
df.plot(x = 'rn', y = 'pt2', ax = ax0p1, style = mkrs[1], label = 'EL-REAL Rate', 
        legend = False, ms = 10, alpha = 0.75)
ax0p1.set_ylim(0.0, 40.0)
ax0p1.set_ylabel('EL-REAL Trigger Rate (kHz)', color = 'g')
...
ax1resid = divider.append_axes("bottom", size = '50%', pad = 0)
ax1.figure.add_axes(ax1resid)
df.plot(x = 'rn', y = 'i4a_i4b_resid', ax = ax1resid, style = 'm-', 
        label = 'BCM4A - BCM4B')
ax1resid.set_ylim(0.0, 1)
ax1resid.set_xlim(2480, 2567)
ax1resid.set_xlabel('Run Number')
ax1resid.set_ylabel('Residual (uA)')
ax1resid.grid()
ax1resid.locator_params(axis = 'y', nbins = 6)
plt.tight_layout()
